...
# groundwork) PLUS a repeat of 0025 (the real stock apple-dart driver,
# dart_apcie1 enabled, no iommu-map) with the power-domains fix
# discovered by 0028's hardware result
...
#
# 0025 hung on real hardware. 0028 (an isolated single write to
# DART+0x24, sharing 0025's missing power-domains reference on
# dart_apcie1) also hung, then booted clean once dart_apcie1 was given
# its own power-domains = <&ps_pcie> reference. This payload restores
# dart_apcie1's real compatible strings (so the stock apple-dart driver
# binds for real: register map, reset, IRQ registration) with that same
# fix applied -- effectively 0025, fixed. CONFIG_APPLE_DART is already
# =y in the base Hoolock defconfig; no kernel config change is needed
...
let
  configuredSource = runCommand "linux-hoolock-pcie-dart-pspcie-fix-test-t7001-source" {} ''
    mkdir -p "$out"
    cp -R ${source}/. "$out/"
    chmod -R u+w "$out"
    cp ${hoolockConfig} "$out/arch/arm64/configs/ipad_t7001_hoolock_pcie_dart_pspcie_fix_test_defconfig"
    ...
    # compile-only skeleton), then 0031 on top -- a clean branch, not
    # stacked on 0025/0027/0028/0029/0030.
    patch -d "$out" -p1 < ${./patches/0016-pcie-apple-t7000-compile-only-skeleton.patch}
    patch -d "$out" -p1 < ${./patches/0031-pcie-apple-t7000-dart-pspcie-fix-test.patch}
  '';
...
buildLinux (buildArgs // {
  ...
  defconfig = "ipad_t7001_hoolock_pcie_dart_pspcie_fix_test_defconfig";
  ...
  extraMeta = {
    branch = "hoolock-pcie-dart-pspcie-fix-test";
    description = "Hoolock kernel for iPad Air 2 (A8X/T7001) with everything kernel/hoolock.nix has plus a repeat of the real stock apple-dart driver test (0025) with dart_apcie1 given its own power-domains = <&ps_pcie> reference -- a dedicated diagnostic payload, not the default";
