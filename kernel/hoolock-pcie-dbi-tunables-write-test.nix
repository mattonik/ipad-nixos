...
# groundwork) plus the hardware-clean PCIe Stages 1-5B and Stage 5C's
# gated RMW. It applies Apple's recovered dbi-overrides (gated through the
# ECAM window at ECAM+0x88bc) and apcie-config-tunables (through the direct
# port-controller window) records, in Apple's own confirmed order.
{ buildLinux
, runCommand
, source
, hoolockConfig
...
let
  configuredSource = runCommand "linux-hoolock-pcie-dbi-tunables-write-test-t7001-source" {} ''
    mkdir -p "$out"
    cp -R ${source}/. "$out/"
    chmod -R u+w "$out"
    cp ${hoolockConfig} "$out/arch/arm64/configs/ipad_t7001_hoolock_pcie_dbi_tunables_write_test_defconfig"
    ...
    # read-only DBI ECAM mapping probe, then Stage 5C's gated RMW. Do not
    # stack historic PCIe tests.
...
buildLinux (buildArgs // {
  ...
  defconfig = "ipad_t7001_hoolock_pcie_dbi_tunables_write_test_defconfig";
  enableCommonConfig = false;
  autoModules = false;
  ignoreConfigErrors = true;
  buildDTBs = true;
  extraMeta = {
    branch = "hoolock-pcie-dbi-tunables-write-test";
    description = "Hoolock kernel for iPad Air 2 (A8X/T7001) with the Stage 5C dbi-overrides/apcie-config-tunables RMW";
