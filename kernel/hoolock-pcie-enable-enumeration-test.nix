...
# groundwork) PLUS Stage 2 of the real PCIe host-controller
# implementation: Stage 1's recovered AppleT7000PCIe::_enablePortHardware
# shared-window write sequence for board port 1, now combined with the
# kernel's generic pci_host_common_init()/ECAM bus scan -- still without
# PERST deassertion, so the scan is expected to find nothing
# (docs/plans/2026-09-13-j81-wifi-pcie.md,
# research/t7000-pcie-hardware-findings.md).
#
# `0033` (Stage 1: the write sequence alone, no enumeration) is
# cross-build verified and awaiting its own hardware result. This test
# adds `pci_host_common_init()` -- already independently proven safe in
# complete isolation by `0023` -- on top of that same write sequence, to
# test whether *combining* them (the generic bus scan also performs
# BAR-sizing config-space writes while walking every device/function
# slot, unlike any bounded read test in this investigation) is safe,
...
let
  configuredSource = runCommand "linux-hoolock-pcie-enable-enumeration-test-t7001-source" {} ''
    mkdir -p "$out"
    cp -R ${source}/. "$out/"
    chmod -R u+w "$out"
    cp ${hoolockConfig} "$out/arch/arm64/configs/ipad_t7001_hoolock_pcie_enable_enumeration_test_defconfig"
    ...
    # PCIe enable-sequence + generic-enumeration test
    # (docs/plans/2026-09-13-j81-wifi-pcie.md): 0016 first (the inert
    # compile-only skeleton), then 0034 on top -- a clean branch, not
    # stacked on 0033 or any other PCIe/DART test.
    patch -d "$out" -p1 < ${./patches/0016-pcie-apple-t7000-compile-only-skeleton.patch}
    patch -d "$out" -p1 < ${./patches/0034-pcie-apple-t7000-enable-enumeration-test.patch}
  '';
...
buildLinux (buildArgs // {
  ...
  defconfig = "ipad_t7001_hoolock_pcie_enable_enumeration_test_defconfig";
  ...
  extraMeta = {
    branch = "hoolock-pcie-enable-enumeration-test";
    description = "Hoolock kernel for iPad Air 2 (A8X/T7001) with everything kernel/hoolock.nix has plus Stage 2 of the real PCIe driver: the recovered AppleT7000PCIe port-1 shared-window enable sequence combined with generic ECAM bus enumeration (still no PERST, no per-port link-start) -- a dedicated diagnostic payload, not the default";
