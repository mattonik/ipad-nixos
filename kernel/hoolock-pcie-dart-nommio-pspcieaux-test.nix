...
# at all), with power-domains = <&ps_pcie_aux> -- the first step of the
...
let
  configuredSource = runCommand "linux-hoolock-pcie-dart-nommio-pspcieaux-test-t7001-source" {} ''
    mkdir -p "$out"
    cp -R ${source}/. "$out/"
    chmod -R u+w "$out"
    cp ${hoolockConfig} "$out/arch/arm64/configs/ipad_t7001_hoolock_pcie_dart_nommio_pspcieaux_test_defconfig"
    ...
    # compile-only skeleton), then 0029 on top -- a clean branch, not
    # stacked on 0025-0028.
    patch -d "$out" -p1 < ${./patches/0016-pcie-apple-t7000-compile-only-skeleton.patch}
    patch -d "$out" -p1 < ${./patches/0029-pcie-apple-t7000-dart-nommio-pspcieaux-test.patch}
  '';
...
buildLinux (buildArgs // {
  ...
  defconfig = "ipad_t7001_hoolock_pcie_dart_nommio_pspcieaux_test_defconfig";
  ...
  extraMeta = {
    branch = "hoolock-pcie-dart-nommio-pspcieaux-test";
    description = "Hoolock kernel for iPad Air 2 (A8X/T7001) with everything kernel/hoolock.nix has plus a DART sibling power-domain isolation test: dart_apcie1 bound to a no-MMIO logging driver with power-domains = <&ps_pcie_aux> -- a dedicated diagnostic payload, not the default";
