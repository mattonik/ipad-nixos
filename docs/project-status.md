...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **DART is the sole remaining common difference vs. the two hanging `0018` attempts.** Offline Ghidra recovery, 2026-09-23, of `AppleEmbeddedPCIE.kext`/`AppleS5L8960XDART.kext` found the previously-documented call order was backwards: Apple calls `function-dart_force_active(true)` *immediately* after requesting the power/clock gate, before confirming the gate is even active and well before any controller register write -- not after port setup. Two real open gaps found: where the DART's own `_manualAvailabilityEnabled` flag gets set (without it, force-active is a no-op), and whether Linux's `power-domains = <&ps_pcie>` binding covers the `PCIE_AUX`/`PCIE_REF` gates the same generic way Apple's PMGR does (only one gate index is ever referenced by name/index in the driver). See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md) for the full decompiled evidence. Next: resolve those two gaps, then a DART-only hardware test. |
