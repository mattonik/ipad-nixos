...
| Charging | **CHG-2 breakthrough, 2026-09-21: real `Charging` status achieved with a single register write, then made convenient to switch.** Three earlier isolated tests established the mechanism: `0x04c0` alone (`0x02`->`0x0a`/200 mA) reduced discharge but wasn't enough to flip the sign; `0x0010` bit 2 alone increased discharge instead, not a charge-enable bit as hoped; both together landed close to the `0x0010`-alone result. A full-kernelcache Ghidra cross-reference pass then genuinely exhausted the static caller trace for the setter (same dead-end shape KLCT's fourth pass hit). Raising `0x04c0` well above the measured drain instead (`0x4a`/1000 mA) produced immediate, stable `Charging` with flat temperature -- the 100 mA default was never a detection failure, just too low a ceiling. A follow-up hypothesis (is `0x0010` a Bluetooth master enable?) was tested and cleanly ruled out. **Approved plan implemented same day**: `boot/ipad_console.py` now has a tier-switcher (`Charging: switch current tier`, 100/500/1000/2100/2400 mA + custom) with a built-in 42 C thermal abort and automatic restore to the validated `0x4a` resting state (not Apple's factory default) -- offline-tested and live-validated against real hardware. Currently **left charging on Martin's explicit instruction**, monitored on a recurring basis with an automatic cutoff (restore to `0x02`, i.e. actually stop, not just revert to the resting tier) at 80% capacity, a 42 C thermal limit, or any other safety concern; full running log in the plan doc. | Stage 2 of the approved plan (a real kernel-level writable sysfs property, `kernel/patches/0017-...`) is designed but not started. Continue monitoring to the 80% cutoff or a safety stop in the meantime. |
