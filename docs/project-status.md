...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed. `0019` through `0023` cleanly verified PMGR attachment, selected shared-window reads, ECAM mapping/reads, and the isolated generic PCI scan. They all kept DART disabled and removed `iommu-map`; both hanging `0018` versions enabled them. Read-only `0018` also accessed four shared offsets that no clean payload isolated. Do not treat the controller writes as the sole suspect. Next: close those four passive reads, then recover DART force-active and PCIE/AUX/REF gate semantics before a DART-only probe. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
