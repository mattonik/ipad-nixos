...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **Enabling DART itself (`0025`, no `iommu-map`) hung on real hardware, 2026-09-23** -- black screen after the m1n1 logo, no USB re-enumeration, no networking. Exact 12B410 tracing now proves the availability helper is the DART provider cast to `AppleARMIODevice`; its clock/power-gate wrappers return unsupported because J81 declares neither array, and Apple ignores both. The next candidate is the write-first `_dartRecoverFromPowerdown()` sequence: Apple writes `DART+0x24 = 0x0020ffff` and restores configuration before Linux reads `+0x00`. **Implemented, cross-build verified, and hardware-tested, 2026-09-24** (`kernel/patches/0027-...`): a new dedicated driver, bound via a redirected `compatible` string so the stock hanging `apple-dart` driver never binds, performs only that one write before aborting probe. **It hung too** -- same signature as `0025`. Since `0027` never reads, only writes, this rules out read-before-write ordering as the cause and points instead at a **different, untested** mechanism: `dart_apcie1` has no `power-domains` property of its own in the DT at all (only `pcie` has one). Whether the DART's own MMIO window needs an explicit `power-domains` reference has never been tried. **Ordered domain-gate plan implemented and cross-build verified, 2026-09-24** (`kernel/patches/0028`-`0030`): Test A (`0028`) gives `dart_apcie1` its own `power-domains = <&ps_pcie>` on top of `0027`'s unchanged single-write driver; Tests B1/B2 (`0029`/`0030`, staged but not to be run unless A hangs) isolate `ps_pcie_aux`/`ps_pcie_ref` individually with a new no-MMIO logging driver, never combined. All three cross-build verified clean. `result` points to Test A; not yet hardware-tested. `0026` stays untested. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
