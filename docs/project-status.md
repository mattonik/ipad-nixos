...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed. `kernel/patches/0018-...` (both versions) hung before a console on hardware, 2026-09-21; a source review found the "read-only" version still mapped ECAM/shared MMIO and ran the generic PCI probe, and PERST was never actually deasserted by either version (`pci_host_common_init()` doesn't call the parse-ports helper). The corrected follow-up, `kernel/patches/0019-...` -- a true PMGR-only probe with zero MMIO, no PCI core, no DART -- **hardware-verified clean, 2026-09-23**: boots exactly like the unmodified control payload (postmarketOS, working USB networking, `dmesg` confirms the probe ran). This rules out the `status = "okay"` DT flip and the automatic `ps_pcie` power-domain attachment as causes -- the real suspect is now `pci_host_common_init()`'s ECAM mapping / generic PCI bus scan, which the working test deliberately never calls. Next: a bounded shared-window MMIO read, still without ECAM. See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#pmgr-only-test-hardware-verified-clean-2026-09-23). |
