...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed; a real link/enumeration-test driver (`kernel/patches/0018-...`) cross-build verified and **hardware-tested, 2026-09-21**: four DFU attempts (write-capable version x2, read-only diagnostic version x1, unmodified control payload x1) show the control payload boots cleanly every time but both PCIe-patch versions hang before reaching a console (black screen, backlight on, no USB networking) -- including the read-only version with zero register writes, which rules out the driver's own logic and isolates the fault to the `status = "okay"` DT flip itself (DART attach or `power-domains = <&ps_pcie>` power-up). See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#first-hardware-boot-attempts-hangs-before-reaching-a-console-2026-09-21). Next: split the DT change to isolate DART from the power domain -- not yet attempted |
