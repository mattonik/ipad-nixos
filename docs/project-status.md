...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **Enabling DART itself (`0025`, no `iommu-map`) hung on real hardware, 2026-09-23** -- black screen after the m1n1 logo, no USB re-enumeration, no networking. The captured textual ADT rules out the initial AUX/REF hypothesis: `dart-apcie1` declares no power/clock gates, but does declare `manual-availability = 1`. Firmware now proves that property sets `_manualAvailabilityEnabled` and `dart_force_active` reaches its virtual availability transition. Next: resolve that handler before another DART payload; keep `0026` untested. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
