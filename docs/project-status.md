...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **Enabling DART itself (`0025`, no `iommu-map`) hung on real hardware, 2026-09-23** -- black screen after the m1n1 logo, no USB re-enumeration, no networking. The captured textual ADT ruled out the initial AUX/REF hypothesis (`dart-apcie1` has no power/clock-gate property, but does have `manual-availability = 1`). **2026-09-24: the availability transition is now fully decompiled, and the gate helper is identified.** `_updateAvailability()`'s virtual dispatch resolves to a real "become available" handler that requests its own dedicated power/clock gate *before* marking itself available or enabling its own interrupt. That gate-request helper is **`AppleARMPerformanceController`** -- identified via its owning kext's load address plus real symbols pulled from a separately-sourced unstripped `AppleARMPlatform.kext` (`enableDeviceClock`/`enableDevicePower`, matching the exact call shapes found) -- the same SoC-wide performance-controller framework already partially reverse-engineered for touch's `KLCT` clock gate. **The DART requests its gate through this framework, not through `ps_pcie`/AUX/REF at all** -- a mechanism Linux's current DT and driver have no representation of whatsoever. Still open: the physical PMGR gate/register these methods resolve to. No further DART hardware payload until that's known; `0026` stays untested. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
