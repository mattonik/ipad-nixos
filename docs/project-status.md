...
| Wi‑Fi implementation | 🟡 **DART and its IOMMU wiring hardware-confirmed working, 2026-09-24** (`kernel/patches/0031`/`0032`): the real, unmodified Linux `apple-dart` driver initializes cleanly on real J81 hardware, and with `pcie`'s `iommu-map` restored the generic IOMMU core confirms a real translated default domain -- the genuine PCIe-to-DART consumer relationship real endpoint drivers like `brcmfmac` would depend on. Root cause of every hang back to `0018`: `dart_apcie1` needs its own `power-domains = <&ps_pcie>` reference -- not a missing gate, not read/write ordering. **Real host-controller driver now being implemented in four single-variable stages, 2026-09-24/25 overnight (explicit authorization, no hardware access)**: Stage 1 (`0033`, the recovered `_enablePortHardware` write sequence alone) and Stage 2 (`0034`, plus generic ECAM enumeration) are both implemented and cross-build verified clean, staged for the user to hardware-test. Stage 3 (`0035`, plus PERST deassertion) is implemented -- caught and fixed a real bug before building, where `pci_host_common_init()` does not automatically deassert PERST as originally assumed -- and its cross-build is in progress. Stage 4 (per-port link-start) not yet started. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
