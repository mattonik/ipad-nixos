...
| Wi‑Fi implementation | 🟡 **DART hardware-confirmed working, 2026-09-24** (`kernel/patches/0031-...`): the real, unmodified Linux `apple-dart` driver now initializes cleanly on real J81 hardware -- `DART [pagesize 1000, 4 streams, bypass support: 0, bypass forced: 0, AS 32 -> 36] initialized` in `dmesg`, real register map/reset/IRQ, postmarketOS boots normally with working networking. Root cause of every earlier hang back to `0018`: `dart_apcie1` needs its own `power-domains = <&ps_pcie>` reference -- `pcie`'s own reference never guaranteed the domain stayed powered for the DART's own, later probe. Not a missing gate, not read/write ordering. **Next**: restore `pcie`'s `iommu-map` (establishing the real PCIe-to-DART IOMMU consumer relationship, `0026`'s original scope) with this fix carried over, before returning to the full controller sequence (DART active → tunables → PERST release → link start → PCI enumeration). See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
