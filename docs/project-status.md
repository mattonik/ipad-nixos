...
| Charging | **CHG-2 breakthrough, 2026-09-21: real `Charging` status achieved with a single register write.** Three earlier isolated tests (below) established the mechanism but not a fix. A full-kernelcache Ghidra cross-reference pass then genuinely exhausted the static caller trace (zero resolvable references to the setter's vtable slot -- the calling object is obtained dynamically at runtime, the same dead-end shape KLCT's fourth pass hit). Martin proposed raising `0x04c0` clearly above the measured drain instead: wrote `0x04c0 = 0x4a` (1000 mA, `0x0010` deliberately left untouched). **`STATUS` transitioned to `Charging` immediately** -- current went positive (`+44000` uA, rising to `+76000` uA over the next minute), voltage rose in step, temperature held flat at 33.6-33.7 C. The 100 mA default was never a detection failure -- it was simply too low a ceiling for input current to ever exceed system draw. Currently **left charging on Martin's explicit instruction**, monitored on a recurring basis with an automatic cutoff (restore to `0x02`) at 80% capacity, a 42 C thermal limit, or any other safety concern; full running log in the plan doc. Earlier tests: `0x04c0` alone (`0x02`->`0x0a`/200 mA) reduced discharge (`-689`->`-567 mA`) but wasn't enough to flip the sign; `0x0010` bit 2 alone increased discharge instead (`-709`->`-809 mA`), not a charge-enable bit as hoped; both together landed close to the `0x0010`-alone result. | Continue monitoring to the 80% cutoff or a safety stop. Once charging is stopped, decide whether raising `0x04c0` should become a permanent part of the payload's boot-time D2207 init, and at what target current. |
