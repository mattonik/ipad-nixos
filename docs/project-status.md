...
| Charging | **CHG-1 hardware-verified, 2026-09-21:** the J81 read-only D2207 child reports 100 mA input and 3 A charge-current limits, matching raw `0x04c0 = 0x02` and `0x04cf = 0x3c`; the simultaneous BQ27545 reading was `Discharging` at about -645 mA. No PMIC write path exists in Linux. **No USB power meter available; two live read-only checks ruled out USB enumeration state as the cause (gadget fully `configured`/`high-speed`) and confirmed zero charger-detection code (`extcon`/`typec`/related `dmesg`) exists in this kernel at all -- the 100 mA reading is almost certainly the D2207's untouched power-on default.** Offline disassembly of the same real iPad5,3 kernelcache then found and fully decoded Apple's own write path: `AppleD2207PMUPowerSource`'s current-limit setter reads `0x04c0`, clamps a target through Apple's classic charger-ID tiers (100/500/1000/2100/2400 mA, each derated a few percent), and writes the result back to `0x04c0` with a confirmed, two-way-verified formula (`mA = (byte & 0x3f) * 50`); a neighboring helper toggles bit 2 of register `0x0010`, plausibly a charge-enable bit. This is an evidence-backed write transaction, not an assumption. | What calls that setter, and how it picks the target current (real charger-type detection), remains untraced -- likely needs the same cross-kext Ghidra technique KLCT's dispatch chain needed. Whether the current evidence justifies a small, reversible, monitored live write test is Martin's call, not decided here. |
