...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **Enabling DART itself (`0025`, no `iommu-map`) hung on real hardware, 2026-09-23** -- black screen after the m1n1 logo, no USB re-enumeration, no networking. Firmware proves that Apple transitions DART availability and invokes two helper operations before active state and interrupt enable. The helper class and physical gate are still open: a reference `AppleARMIODevice` gate-wrapper path exactly fits the calls, but J81's DART has no gate properties. Resolve the exact 12B410 helper callee before another DART payload; `0026` stays untested. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
