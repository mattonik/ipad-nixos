...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed. `kernel/patches/0018-...` (both versions) hung before a console on hardware, 2026-09-21; PERST was never actually deasserted by either version. Four corrected follow-ups have now **hardware-verified clean, 2026-09-23**: `0019` (PMGR-only), `0020` (shared-window read), `0021` (ECAM offset-0 read), and `0022` (ECAM reads at bus 0/1/4) all boot exactly like the unmodified control payload, `dmesg`-confirmed at every step -- every raw MMIO read tried anywhere on this controller is safe, ruling out an "unsafe region" theory entirely. Only `pci_host_probe()`'s actual bus-scan logic remains untested -- `pci_host_common_init()` sets `PCI_REASSIGN_ALL_BUS` and generic enumeration performs writes (BAR-sizing, bus-number programming) that no read-only test can exercise. Next: call `pci_host_common_init()` in full isolation (no register writes, no PERST/GPIO, no DART) to test that one remaining code path directly. See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#multi-offset-ecam-read-test-hardware-verified-clean-2026-09-23). |
