...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed. `kernel/patches/0018-...` (both versions) hung before a console on hardware, 2026-09-21; PERST was never actually deasserted by either version. Three corrected follow-ups have now **hardware-verified clean, 2026-09-23**: `0019` (PMGR-only), `0020` (bounded shared-window read, LTSSM reads sane `0x00000000`), and `0021` (bounded ECAM-window read, bus0/dev0/fn0 config-space read returns the correct all-Fs "no device" response) all boot exactly like the unmodified control payload, `dmesg`-confirmed at every step. This rules out the DT status flip, the power-domain attachment, the shared-window MMIO access, and a single ECAM read as causes. Only `pci_host_probe()`'s full generic bus scan remains untested; leading hypothesis is that only part of the declared 16 MiB ECAM window is actually safe to touch, and a full multi-bus scan reaches an unmapped region a single offset-0 read doesn't. Next: bounded reads at other ECAM offsets (higher bus numbers / other device-function slots) to localize a bad region before re-running the full scan. See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#bounded-ecam-window-read-test-hardware-verified-clean-2026-09-23). |
