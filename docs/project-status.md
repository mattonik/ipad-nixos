...
| Charging | **CHG-1/CHG-2, 2026-09-21: three clean, isolated, fully-reversible live write tests, none produced actual charging.** CHG-1 confirmed the read-only child correct against raw registers (`0x04c0 = 0x02` = 100 mA, `0x04cf = 0x3c` = 3 A max). Offline disassembly of the real iPad5,3 kernelcache found Apple's own write path (`AppleD2207PMUPowerSource`'s current-limit setter, `0x04c0`) and a neighboring charge-enable-bit candidate (`0x0010` bit 2). Live-tested all three ways, each with explicit authorization, each isolated and restored immediately after: (1) `0x04c0` alone (`0x02`->`0x0a`) **worked as a real write** (unlike GPIO2) and measurably *reduced* discharge (`-689`->`-567 mA`); (2) `0x0010` bit 2 alone also wrote and persisted, but *increased* discharge instead (`-709`->`-809 mA`), no `STATUS` change -- not a charge-enable bit as hoped; (3) both together landed close to result (2), not better -- the bit's penalty roughly cancels `0x04c0`'s benefit rather than combining toward charging. All three fully restored, `dmesg` clean throughout, current settled back to baseline within seconds each time. | Trace the current-limit setter's real caller (still-untraced vtable dispatch) -- the only remaining lead for what actually triggers real charger-type detection/enable, since neither register tested so far does it alone or together. |
