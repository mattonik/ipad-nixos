...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **DART is the sole remaining common Linux-configuration difference vs. the two hanging `0018` attempts.** Ghidra recovery found Apple requests `function-dart_force_active(true)` immediately after its power/clock-gate requests, before controller writes. The pinned Linux PMGR source then resolved the Linux gate question: `ps_pcie`, `ps_pcie_aux`, and `ps_pcie_ref` are independent genpds, so the current `power-domains = <&ps_pcie>` covers only the first. Next: retain the safe PCIe PMGR-only node, enable DART without `iommu-map`; if clean, restore only `iommu-map`. AUX/REF ownership and Apple's `_manualAvailabilityEnabled` remain production-controller work. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
