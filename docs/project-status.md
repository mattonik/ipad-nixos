...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed; a link/enumeration-test driver (`kernel/patches/0018-...`) cross-build verified and **hardware-tested, 2026-09-21**: both PCIe-patch versions hung before a console while the control payload booted. A subsequent source review corrected the original diagnosis: the supposed read-only version still mapped ECAM/shared MMIO and entered the generic PCI probe, while the first version never deasserted PERST or completed Apple's link-start sequence. Do not rerun it. First recover PCIE/AUX/REF gate ownership and DART force-active semantics, then use a true PMGR-only no-MMIO payload. See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#post-attempt-implementation-review-the-diagnostic-was-not-passive-2026-09-21). |
