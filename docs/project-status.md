...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **Enabling DART itself (`0025`, no `iommu-map`) hung on real hardware, 2026-09-23** -- black screen after the m1n1 logo, no USB re-enumeration, no networking, the same signature as both original `0018` hangs. `0019`'s PCIe PMGR-only node is independently proven clean six times over and unchanged here, so the fault isolates to `dart_apcie1` alone. Leading hypothesis: `dart_apcie1` has no `power-domains` property at all, and the pinned PMGR DTS defines `ps_pcie_aux`/`ps_pcie_ref` genpd nodes nothing in the current DT references -- if the DART's silicon needs AUX/REF rather than (or in addition to) the one gate this test enables, its first real register touch (during the stock `apple-dart` driver's reset step) would hit unclocked hardware. Test 2 (`0026`, `iommu-map` restored) does **not** run next per the plan's own conditional gate -- it did not boot. Next is research (ADT binary parsing / another Ghidra pass on `AppleS5L8960XDART`) to identify the DART's real gate dependency before any further hardware attempt. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
