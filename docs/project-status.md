...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed. `kernel/patches/0018-...` (both versions) hung before a console on hardware, 2026-09-21; PERST was never actually deasserted by either version (`pci_host_common_init()` doesn't call the parse-ports helper). Two corrected follow-ups have now **hardware-verified clean, 2026-09-23**: `0019` (PMGR-only, zero MMIO, no PCI core, no DART) and `0020` (adds a real `devm_ioremap_resource()` + single bounded `readl()` of the shared register window, still no ECAM/PCI core/DART) both boot exactly like the unmodified control payload, with `dmesg` confirming each step ran (`0020`'s port-1 LTSSM register reads a sane `0x00000000`). This rules out the `status = "okay"` DT flip, the automatic `ps_pcie` power-domain attachment, and the shared-window MMIO access itself as causes. The real suspect is now narrowed to `pci_host_common_init()`'s remaining two pieces -- ECAM mapping and the generic PCI bus scan -- which both hanging `0018` attempts called and neither working test does. Next: a bounded ECAM-only read, still without the generic bus scan. See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#bounded-shared-window-read-test-hardware-verified-clean-2026-09-23). |
