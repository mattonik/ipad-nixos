...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean). `0025` (DART enabled, stock `apple-dart` driver, no `power-domains` of its own) and `0027` (isolated single write to `DART+0x24`, same gap) both hung on real hardware. **Root cause found and hardware-confirmed clean, 2026-09-24** (`kernel/patches/0028`): giving `dart_apcie1` its own `power-domains = <&ps_pcie>` reference -- not a missing AUX/REF gate, not read/write ordering, but genpd power-up ordering scoped per consumer device (`pcie`'s own reference never guaranteed `ps_pcie` stayed powered for the DART's own, later probe). `dmesg` confirms the write completed in 14 microseconds and boot continued normally to postmarketOS with working networking. Tests B1/B2 (`0029`/`0030`, AUX/REF isolation) are no longer needed. **Implemented and cross-build verified, 2026-09-24** (`kernel/patches/0031-...`): the real stock `apple-dart` driver restored on `dart_apcie1` (compatible strings undone from the test redirect) with the same `power-domains = <&ps_pcie>` fix -- effectively `0025` with the fix applied. `result` points to this payload; not yet hardware-tested. `0026` stays untested until then. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
