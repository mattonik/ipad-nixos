...
| Wi‑Fi implementation | 🟡 **DART and its IOMMU wiring hardware-confirmed working, 2026-09-24** (`kernel/patches/0031`/`0032`): the real, unmodified Linux `apple-dart` driver initializes cleanly on real J81 hardware. Root cause of every hang back to `0018`: `dart_apcie1` needs its own `power-domains = <&ps_pcie>` reference. **Real host-controller driver, staged single-variable steps, 2026-09-25**: Stage 1 (write sequence), Stage 2 (generic PCI enumeration -- root port self-identified with Apple's vendor ID), and Stage 3 (corrected PERST deassertion) are all hardware-tested clean. Stage 4a (`0036`) hit a real, informative `-EBUSY` -- the per-port controller window physically overlaps `dart_apcie1`'s own MMIO region, not a hang, not a bug in the recovered index -- fixed as `0038` and hardware-tested clean (window reads `0x00000000`, a real non-fault value). **Full recovered enable sequence (Stages 1-4b v2) now hardware-confirmed safe end to end, 2026-09-25.** Stage 4b v2's link-start write genuinely took and stuck, with no crash or hang anywhere in the now-complete staged sequence. Bus enumeration still found no downstream WiFi endpoint after the write, even across three separate scan attempts. **Stage 5A (`0040`), from Martin's own plan-B research, is hardware-tested clean, 2026-09-25**: all seven `apcie-config-tunables`/`dbi-overrides` offsets read real, plausible baseline values (two non-zero, rest zero), no faults. Applying Apple's recorded RMW to this baseline produces well-formed deltas at every offset. Stage 5B applies those records for real once the remaining ordering/DBI-gate question is resolved. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
