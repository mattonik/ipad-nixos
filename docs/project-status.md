...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed. `0019` through `0024` cleanly verified PMGR attachment, every shared-window register the enable sequence touches (including the four `0020` had missed, real "at rest" values now recorded), ECAM mapping/reads, and the isolated generic PCI scan. All six kept DART disabled and removed `iommu-map`; both hanging `0018` versions enabled them. **DART is now the sole remaining common difference between the hanging attempts and every clean test.** Next (a research task, not a hardware test): recover Apple's `function-dart_force_active` semantics and the PCIE/AUX/REF power-gate operations offline, then run one DART-only probe (DART enabled, PCIe inert, no IOMMU consumer) before ever attempting controller writes, PERST, or link training. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
