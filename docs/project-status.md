...
| Wi‑Fi implementation | 🟡 Real J81 resources and the generic PCI path are hardware-confirmed (`0019`-`0024`, all clean, DART disabled throughout). **Enabling DART itself (`0025`, no `iommu-map`) hung on real hardware, 2026-09-23** -- black screen after the m1n1 logo, no USB re-enumeration, no networking. The captured textual ADT ruled out the initial AUX/REF hypothesis (`dart-apcie1` has no power/clock-gate property, but does have `manual-availability = 1`). **2026-09-24: the availability transition is now fully decompiled.** `_updateAvailability()`'s virtual dispatch resolves to a real "become available" handler that requests its own dedicated power/clock gate (via a cached helper object, same two-call shape as the PCIe port's `enableGated()`) *before* marking itself available or enabling its own interrupt -- a mechanism the stock Linux `apple-dart` driver has no equivalent of at all, giving a concrete explanation for the hang. Still open: the helper object's own class, i.e. which PMGR gate it actually requests. No further DART hardware payload until that's known; `0026` stays untested. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
