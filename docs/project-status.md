...
| Wi‑Fi implementation | 🟡 Real J81 resources, PCI apertures and DART compatibility confirmed. `kernel/patches/0018-...` (both versions) hung before a console on hardware, 2026-09-21. Five corrected follow-ups have now **hardware-verified clean, 2026-09-23**: `0019` (PMGR-only), `0020` (shared-window read), `0021` (ECAM offset-0 read), `0022` (ECAM at bus 0/1/4), and `0023` (the full, isolated `pci_host_common_init()` call -- ECAM mapping plus the real `pci_host_probe()` generic bus scan, including its BAR-sizing writes -- with no shared-window writes, PERST, or DART) all boot exactly like the unmodified control payload, `dmesg`-confirmed at every step, `0023` explicitly returning `0`. **This conclusively isolates the fault**: every piece of `pci_host_common_init()` is now individually proven safe. The only thing either hanging `0018` attempt did that none of these five tests did is the actual hardware enable-sequence writes to the shared window (`REFCLK_EN`, `PERST_INTERNAL`, the undocumented `0x10c`, `LINK_ENABLE`, and the final LTSSM-start write) -- never tested in isolation. Next: those writes, staged one at a time. See [full writeup](plans/2026-09-13-j81-wifi-pcie.md#isolated-pci_host_common_init-test-hardware-verified-clean-2026-09-23). |
