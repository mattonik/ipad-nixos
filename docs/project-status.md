...
| Wi‑Fi implementation | 🟡 **DART and its IOMMU wiring hardware-confirmed working, 2026-09-24** (`kernel/patches/0031`/`0032`): the real, unmodified Linux `apple-dart` driver initializes cleanly on real J81 hardware (`DART [pagesize 1000, 4 streams, bypass support: 0, bypass forced: 0, AS 32 -> 36] initialized`), and with `pcie`'s `iommu-map` restored the generic IOMMU core confirms a real translated default domain (`iommu: Default domain type: Translated`) -- the genuine PCIe-to-DART consumer relationship real endpoint drivers like `brcmfmac` would depend on. postmarketOS boots normally with working networking in both cases. Root cause of every hang back to `0018`: `dart_apcie1` needs its own `power-domains = <&ps_pcie>` reference -- not a missing gate, not read/write ordering. `pcie` itself is still `0019`'s inert stand-in probe; no link training, controller writes, PERST, or enumeration yet. **Next, not yet decided**: implementing the real PCIe host-controller driver (`_enablePortHardware` sequence, PERST, DART already proven safe) -- materially larger in scope than any test so far, worth its own explicit go-ahead. See [the consolidated PCIe record](../research/t7000-pcie-hardware-findings.md). |
