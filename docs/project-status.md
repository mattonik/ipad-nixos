...
| Charging | **CHG-1 hardware-verified, then CHG-2's write path found and live-tested, 2026-09-21.** CHG-1: the J81 read-only D2207 child reports 100 mA input and 3 A charge-current limits, matching raw `0x04c0 = 0x02` and `0x04cf = 0x3c`. No USB power meter available; two live read-only checks ruled out USB enumeration state (gadget fully `configured`/`high-speed`) and confirmed zero charger-detection code exists in this kernel at all -- the 100 mA reading is the D2207's untouched power-on default. Offline disassembly of the real iPad5,3 kernelcache found `AppleD2207PMUPowerSource`'s current-limit setter: reads `0x04c0`, clamps a target through Apple's classic charger-ID tiers, writes the result back to `0x04c0` -- the same register CHG-1 validated, via the same I2C vtable ABI already proven for GPIO2. **Live-tested with explicit authorization**: wrote `0x04c0 = 0x0a` (500 mA-tier code) over the USB shell; readback confirmed `0x0a` and **the write persisted** (unlike GPIO2's ACKed-but-unchanged result) -- sysfs decoded it to 200 mA, and the gauge's discharge current measurably dropped (`-689 mA` -> `-567 mA`), a real ~120 mA change, not just a cosmetic register. Restored to `0x02` immediately after; readback and sysfs confirmed clean restoration, `dmesg` showed no errors throughout. **Then tested `0x0010` bit 2 the same isolated way** (with `0x04c0` confirmed unchanged at `0x02` throughout, so any effect is attributable to this one register): baseline `0x00`, wrote `0x04` (set bit 2), readback confirmed it took and persisted. But the "charge-enable" hypothesis was **not** supported: `STATUS` stayed `Discharging` the whole time, and discharge current moved the *wrong* direction (`-709 mA` -> `-809 mA`, roughly 100 mA of *extra* draw, not reduced) -- and settled back to baseline (`-701 mA`) within 8s of restoring, good evidence the bump really was caused by the write. Restored to `0x00` cleanly. **Conclusion: real, writable bit, but not confirmed as charge-enable** -- more consistent with gating some quiescent-current-drawing circuit (comparator/regulator/detection block) than connecting USB current through to the battery. | Try the same isolated procedure with `0x04c0` *also* raised at the same time -- maybe the combination, not either register alone, produces net charging current. Separately, trace what upstream code calls the current-limit setter (real charger-type detection), likely needing the cross-kext Ghidra technique KLCT's dispatch chain needed. |
