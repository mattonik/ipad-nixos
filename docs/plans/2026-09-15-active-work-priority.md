...
| --- | --- | ---: | ---: | ---: | --- |
| 1 | PCIe: implement the real host-controller driver | 5 | 2 | 5 | In progress, 2026-09-25: implementing in staged single-variable steps, all five cross-build verified clean -- Stage 1 (`0033`, write sequence alone), Stage 2 (`0034`, plus generic ECAM enumeration), Stage 3 (`0035`, plus PERST -- a design bug, assumed-automatic PERST deassertion, was caught and fixed before building), Stage 4a (`0036`, bounded read of the untouched per-port controller window), Stage 4b (`0037`, the link-start write, staged ahead of Stage 4a's own hardware gate -- do not test before Stage 4a reads clean). **Stage 1 and Stage 2 hardware-tested clean, 2026-09-25**: both booted normally, and Stage 2 confirmed real generic PCI enumeration works (root port self-identified with Apple's vendor ID). Proceeding to Stage 3. |
