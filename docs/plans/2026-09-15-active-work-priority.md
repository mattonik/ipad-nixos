...
| --- | --- | ---: | ---: | ---: | --- |
| 1 | PCIe: implement the real host-controller driver | 5 | 2 | 5 | In progress, 2026-09-24/25 overnight (explicit go-ahead + "continue overnight" authorization): implementing in staged single-variable steps. Stages 1-3 all implemented and cross-build verified clean, staged for hardware-test in sequence: Stage 1 (`0033`, write sequence alone), Stage 2 (`0034`, plus generic ECAM enumeration), Stage 3 (`0035`, plus PERST -- a design bug, assumed-automatic PERST deassertion, was caught and fixed before building). Stage 4a (`0036`, bounded read of the untouched per-port controller window, no write) also implemented and cross-build verified clean. Stage 1 (`0033`) is the correct first hardware test of this whole sequence -- none of the five have been hardware-tested yet, and the project's own single-variable discipline means each stage is only tested after the previous one boots clean; `result` has been corrected to point at Stage 1 accordingly. Stage 4b (`0037`, the link-start write) implemented and cross-build verified clean (after resolving a real Nix store corruption spanning local + remote builder, see `docs/build-infrastructure.md`), staged ahead of Stage 4a's own hardware gate, matching this project's build-ahead precedent; must not be hardware-tested before Stage 4a reads clean. Cross-build-only, no hardware access this pass. |
