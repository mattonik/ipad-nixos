...
| --- | --- | ---: | ---: | ---: | --- |
| 1 | PCIe: implement the real host-controller driver | 5 | 2 | 5 | In progress, 2026-09-24/25 overnight (explicit go-ahead + "continue overnight" authorization): implementing in four single-variable stages. Stage 1 (`0033`, the recovered `_enablePortHardware` write sequence alone, no PERST/enumeration) is implemented and cross-build verified clean, staged for the user to hardware-test. Stages 2 (generic ECAM enumeration) through 4 (PERST, per-port link-start) in progress, cross-build-only, no hardware access. |
