...
| --- | --- | ---: | ---: | ---: | --- |
| 1 | PCIe: implement the real host-controller driver | 5 | 2 | 5 | In progress, 2026-09-25: implementing in staged single-variable steps, all five cross-build verified clean -- Stage 1 (`0033`, write sequence alone), Stage 2 (`0034`, plus generic ECAM enumeration), Stage 3 (`0035`, plus PERST -- a design bug, assumed-automatic PERST deassertion, was caught and fixed before building), Stage 4a (`0036`, bounded read of the untouched per-port controller window), Stage 4b (`0037`, the link-start write, staged ahead of Stage 4a's own hardware gate -- do not test before Stage 4a reads clean). **Stages 1-3 hardware-tested clean, 2026-09-25**: all booted normally; Stage 2 confirmed real PCI enumeration works, Stage 3 confirmed the corrected PERST-handling code genuinely runs. **Stage 4a hit a real, informative, non-fatal `-EBUSY`** (per-port controller window physically overlaps `dart_apcie1`'s own already-claimed MMIO region, not a hang, not a bug in the recovered index) -- fixed as `0038` (non-exclusive mapping), cross-build verified clean -- next payload to hardware-test. |
