...
| --- | --- | ---: | ---: | ---: | --- |
| 1 | PCIe: implement the real host-controller driver | 5 | 2 | 5 | In progress, 2026-09-24/25 overnight (explicit go-ahead + "continue overnight" authorization): implementing in staged single-variable steps. Stages 1-3 all implemented and cross-build verified clean, staged for hardware-test in sequence: Stage 1 (`0033`, write sequence alone), Stage 2 (`0034`, plus generic ECAM enumeration), Stage 3 (`0035`, plus PERST -- a design bug, assumed-automatic PERST deassertion, was caught and fixed before building). Stage 4a (`0036`, bounded read of the untouched per-port controller window, no write) also implemented and cross-build verified clean; the link-start write is deferred to Stage 4b. Cross-build-only, no hardware access this pass. |
| 2 | Charging Stage 2: writable `input_current_limit` kernel property | 5 | 4 | 3 | Implemented, 2026-09-25 (`kernel/patches/0017-...`, isolated `m1n1-hoolock-charging-writable-test` payload, mirroring the ANS1-test pattern). Verified byte-exact via reconstruct/diff/verify. **Cross-build blocked**, not a code problem: the Linux-builder VM's 40 GB disk filled from five back-to-back kernel builds this session; freeing it needs `sudo` credentials this autonomous session doesn't have. See `docs/build-infrastructure.md`'s "Disk exhaustion from many builds in one session" section. |
