...
| --- | --- | ---: | ---: | ---: | --- |
| 1 | PCIe: implement the real host-controller driver | 5 | 2 | 5 | In progress, 2026-09-24/25 overnight (explicit go-ahead + "continue overnight" authorization): implementing in four single-variable stages. Stage 1 (`0033`, write sequence alone) and Stage 2 (`0034`, plus generic ECAM enumeration) both implemented and cross-build verified clean, staged for hardware-test. Stage 3 (`0035`, plus PERST) implemented -- a design bug (assumed-automatic PERST deassertion) caught and fixed before building -- cross-build in progress. Stage 4 (per-port link-start) not started. Cross-build-only, no hardware access this pass. |
