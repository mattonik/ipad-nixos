...
0 of the *per-port controller window's* own config register at offset
`0x80` to start the link attempt -- but that window (ADT index `2 *
port + 1` = 3 for port 1) has never been touched by any test in this
investigation, unlike the shared/ECAM windows which both had many
bounded reads before any write. Applying that same discipline: `0036`
adds one bounded, logged read (offsets `0x00` and `0x80`) of the
per-port window, still no write -- the link-start write is a later,
separately gated stage (4b). Also explicitly resolved an apparent
labeling conflict before trusting the ADT index: the plan doc's own
register table labels index 3 as "port 2" (by literal address digit),
which looked like it could contradict using it for "port 1," but that's
